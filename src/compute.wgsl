...
fn lcp(i: i32, j: i32) -> i32 {
    ...
    let zeroes = countLeadingZeros(particles[i].morton.y ^ particles[j].morton.y);
    if (zeroes == 32) {
        return i32(zeroes + countLeadingZeros(particles[i].morton.x ^ particles[j].morton.x));
    } else {
        return i32(zeroes);
    }
}
...
@compute @workgroup_size(1) fn physics(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    particle.velocity += particle.acceleration * 0.1;
